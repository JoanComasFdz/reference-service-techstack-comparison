# Java 21 Spring Boot Reference Service

Java 21 Spring Boot Reference Service - a barebones implementation using Java 21 and Spring Boot 3.x for the Performance Test.
...
- `java21.events` library (built from `../java21.events`)

## Database Setup

The service uses a dedicated database `java21springboot_db`. Create it before running the service:
...
# Create the database
CREATE DATABASE java21springboot_db;
```
...
First, build the shared `java21.events` library:
...
### VS Code
1. Open the `java21springboot` folder in VS Code
2. Install the "Extension Pack for Java" extension
3. Press F5 or use the Run and Debug panel
4. Select "Debug Spring Boot App"

### IntelliJ IDEA
1. Open the `java21springboot` folder as a Maven project
2. Right-click on `Java21SpringBootReferenceServiceApplication.java`
3. Select "Debug 'Java21SpringBootReferenceServiceApplication.main()'"

## API Endpoints

- `GET http://localhost:8097/kpi` - Returns the latest instrument status record from the database (not all records)
...
### Startup Time
```bash
# Measure time from start to "Started Java21SpringBootReferenceServiceApplication"
time mvn spring-boot:run
# Or with JAR:
time java -jar target/javaSpringBootReferenceService21.jar
```
...
# Check memory at start (after "Started Java21SpringBootReferenceServiceApplication" log)
...
- `Java21SpringBootReferenceServiceApplication.java` - Main entry point
...
This service uses the shared `java21.events` library for CloudEvents v1.0:
- **Library**: `com.joancomasfdz.java21.events:java21.events:1.0.0`
- **Location**: `../java21.events`
- **Class**: `com.joancomasfdz.java21.events.CloudEvent`
