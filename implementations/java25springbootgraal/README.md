# Performance Test - Java 25 Spring Boot with GraalVM Native Image

This is a GraalVM-enabled version of the Java 25 Spring Boot reference service. It provides the same functionality as the standard JVM version (`java25springboot`) but can be compiled to a native executable using GraalVM Native Image.
...
1. **Java 25** - Required for both regular and native builds
2. **GraalVM for Java 25** (or GraalVM Native Image tools)
3. **Maven** - Build tool
...
setx JAVA_HOME "C:\graalvm\graalvm-jdk-25"
...
### Option 2: Add Native Image to Existing Java 25 JDK
...
```
java25springbootgraal/
├── src/
│   └── main/
│       ├── java/
│       │   └── com/joancomasfdz/performancetest/
│       │       ├── Java25SpringBootGraalReferenceServiceApplication.java      # Main application
...
1. **Queue Name**: Uses `javaSB25Graal` instead of `javaSB` (to avoid conflicts with Java 21 version)
2. **Server Port**: Runs on port `8102` (assigned port for this implementation)
3. **Event Source**: Identifies as `urn:uuid:java25-springboot-graal`
...
# Run
java -jar target/javaSpringBootGraalReferenceService25.jar
```
...
# target/javaSpringBootGraalReferenceService25.jar

# Run the native executable (if buildpacks approach is used, this generates a JAR)
java -jar target/javaSpringBootGraalReferenceService25.jar
```
...
```bash
# The service runs on port 8102
curl http://localhost:8102/kpi
```
...
- **Server port**: 8102 (different from standard version to allow parallel running)
- **Queue name**: `javaSBGraal` (different from standard version)
- **Database**: `java25springbootgraal_db`
- **RabbitMQ**: Same as other services
...
cd ../java25springbootgraal
./target/javaSpringBootGraalReferenceService25
