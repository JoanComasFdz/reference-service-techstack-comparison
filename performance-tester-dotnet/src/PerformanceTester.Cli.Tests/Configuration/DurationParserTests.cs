...
public class DurationParserTests
{
    ...
    #region TryParse - Valid Inputs

    [Theory]
    [InlineData("1s", 1)]
    [InlineData("30s", 30)]
    [InlineData("60s", 60)]
    public void TryParse_ValidSeconds_ReturnsTrueAndCorrectTimeSpan(string input, int expectedSeconds)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [InlineData("5m", 5)]
    [InlineData("30m", 30)]
    public void TryParse_ValidMinutes_ReturnsTrueAndCorrectTimeSpan(string input, int expectedMinutes)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [InlineData("2h", 2)]
    [InlineData("24h", 24)]
    public void TryParse_ValidHours_ReturnsTrueAndCorrectTimeSpan(string input, int expectedHours)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [InlineData("5M")]
    [InlineData("2H")]
    public void TryParse_UppercaseUnits_ReturnsTrueAndCorrectTimeSpan(string input)
    {
        var success = DurationParser.TryParse(input, out var result);

        success.Should().BeTrue();
        result.Should().NotBe(TimeSpan.Zero);
    }
    ...
    [InlineData("30s  ")]
    [InlineData("  30s  ")]
    public void TryParse_WithWhitespace_ReturnsTrueAndCorrectTimeSpan(string input)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [InlineData("")]
    [InlineData("   ")]
    public void TryParse_NullOrEmpty_ReturnsFalseAndZero(string? input)
    {
        var success = DurationParser.TryParse(input!, out var result);
    ...
    [InlineData("-30s")]
    [InlineData("s30")]
    public void TryParse_InvalidFormat_ReturnsFalseAndZero(string input)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [InlineData("1w")]
    [InlineData("1y")]
    public void TryParse_InvalidUnit_ReturnsFalseAndZero(string input)
    {
        var success = DurationParser.TryParse(input, out var result);
    ...
    [Fact]
    public void TryParse_InvalidInput_DoesNotThrow()
    {
        // This test verifies TryParse does not throw for any invalid input
        var invalidInputs = new[] { null!, "", "   ", "30", "s", "30 s", "30.5s", "-30s", "s30", "30d", "abc" };

        foreach (var input in invalidInputs)
        {
            var action = () => DurationParser.TryParse(input, out _);
            action.Should().NotThrow($"TryParse should not throw for input '{input ?? "null"}'");
        }
    }
    ...
    [Fact]
    public void TryParse_ZeroValue_ReturnsTrueAndZeroTimeSpan()
    {
        var success = DurationParser.TryParse("0s", out var result);
